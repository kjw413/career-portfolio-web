...
## 부록 0-1. 구현 현황 (2026-09-12)

Phase 0~5를 모두 구현했습니다. 브랜치 `claude/optimistic-maxwell-p7cr96`.

| Phase | 상태 | 남은 것 |
|---|---|---|
| 0 불일치 수정 | 완료 | — |
| 1 원장 구조화 + 검증 CI | 완료 | — |
| 2 UI v2 | 완료 | Lighthouse 실측 (브라우저 엔진이 있는 환경 필요) |
| 3 3D 첫 화면 | 완료 | — |
| 4 드라이브 연동 | 스크립트 완료 | Apps Script 설치와 트리거 등록은 사용자가 드라이브에서 1회 수행 |
| 5 AI 자소서 스킬 | 완료 | — |

사용자가 직접 해야 하는 것은 두 가지입니다.
`scripts/drive/sync-ledger.gs`를 드라이브 Apps Script에 넣고 일 단위 트리거를 거는 것,
그리고 개인정보 부록 문서를 드라이브에 따로 두는 것입니다. 둘 다 저장소에서 할 수 없습니다.
...
| 3D는 첫 화면(hero) 안에 | 첫 화면 바로 아래의 독립 띠로 분리 | 첫 화면의 글과 수치를 가리지 않고, LCP 후보에서 빠진다 |
| 공장 블록 hover 툴팁 | 넣지 않음 | 장면 폭이 좁아 라벨이 읽히지 않는다. 공장 이름은 캡션과 대체 텍스트에 넣었다 |
| 포스터 1장 | 밝은 화면·어두운 화면 2장 | 다크 모드에서 밝은 포스터만 남으면 페이지와 따로 논다 |
| 스크롤에 따라 카메라가 물러남 | 넣지 않음 | 장면이 전체 화면이 아니라 띠라 필요가 줄었고, 옆 글을 읽는 동안 화면이 움직이면 방해가 된다. 포인터 시차만 남겼다 |
| 웹폰트를 stylesheet로 로드 | `media="print"`로 받아 렌더를 막지 않고, 다 받은 뒤 적용 | 검증에서 CDN이 3초 늦으면 첫 페인트도 3.3초 늦는 것을 확인했다. 지금은 CDN 상태와 무관하게 약 300ms |
| 오프스크린에서 장면을 떼어 냄 | 캔버스는 두고 렌더 루프만 재움 | 스크롤할 때마다 WebGL 컨텍스트를 다시 만드는 비용이 루프를 재우는 것보다 크다 |
| 공장 블록 5개를 호 위에 놓고 가운데 원반 노드로 모음 | 남한 지도 위 실제 위치(시 단위)에 세우고, 지도 서쪽에 띄운 화면(대시보드 판) 한 점으로 모음. 예측 구간 리본은 그 화면 안의 차트로 | 사용자 요청(2026-09-12). 첫 판은 뜻 없는 블록 높이가 뜻이 있어 보였고, 길 없는 알갱이가 어디로 가는지 읽히지 않았으며, 원반 기둥이 가운데 공장에서 솟는 것처럼 보여 "중앙 시스템이 한 공장에 있다"로 읽혔다. 지도는 Natural Earth 공개 데이터를 좌표 배열로 넣어 외부 자산 없음 원칙을 지켰다 |
| 지도 서쪽의 화면 판 하나 | 지도 위 허공의 원형 패널 링(통합 화면) + 어두운 홀로그램 무대. 지도·아이콘·패널은 캔버스 네온 텍스처 | 사용자 제공 참고 이미지(2026-09-12). "다섯 곳이 하나의 통합 시스템으로 모인다"가 링 가운데로 모이는 그림에서 더 강하게 읽힌다. 무대는 테마와 무관하게 어둡다 — 홀로그램은 밝은 배경에서 빛나지 않는다 |
| 공장 5개 = 남양주1·남양주2·김해·광주·논산 | 남양주(1·2)·김해·논산·광주(경기)·경산 | 사용자 확정(2026-09-12). 경산이 빠져 있었고 광주는 경기도 광주다. 옛 표기는 `bems-plants.deprecated`에 남겨 검사기가 잡는다 |
| 장면 설명: "BEMS 대시보드가 보여 주는 것을 그대로 옮긴 장면" | "BEMS가 다루는 것을 지도 위에 도식화한 장면" + 범례 + 원장 수치(예측 오차) 인용 | "그대로 옮긴"은 확인할 수 없는 과장이다. 원장이 금지하는 종류의 표현을 사이트 문구가 쓰고 있었다 |
